...
{
	//by Ahmed Saif
	public $is_selectable = false;
	public $row_classes = array();
	public $custom_theme_config = array();
	public $relation_callback = array();
	public $button_condition = array();
	public $detail_buttons_position = array('both','left');
	public $custom_save = false;
	//end by Ahmed Saif
	...
	// by Ahmed Saif {
    public function after_resize($callable = '', $path = 'functions.php')
    {
        if ($callable && $path)
        {
            $this->after_resize['callable'] = $callable;
            $this->after_resize['path'] = $path;
        }
        return $this;
    }
	// } By Ahmed Saif
    ...
    protected function _update($postdata, $primary)
    {
        ...
        else
        {
            ...
            if (!$this->demo_mode)
				$this->where_pri($this->primary_key, $this->primary_val); // Add by Saif
				$where = $this->_build_where(); // Added By Saif
                $res = $db->query("UPDATE `{$this->table}` AS `{$this->table}` " . implode(' ', $joins) . " SET " . implode(",\r\n", $set) ." ". $where);
                  //  " WHERE `{$this->table}`.`{$this->primary_key}` = " . $db->escape($primary)); //Removed by Saif
        }
        ...
        if ($this->instance_count == 1 && !$this->ajax_request)
        {
            if (isset($_SESSION['lists']['xcrud_session']) && $_SESSION['lists']['xcrud_session'])
            {
                ...
                { // workaround on some servers session duplication
                    ...
                    //if ($time > $old_time + Xcrud_config::$autoclean_timeout) // autocleaner removed by saif
    ...
    protected function params2save()
    {
        return array(
            ...
            'custom_fields',
            'date_format',
			'is_selectable', // by saif
			'row_classes', //by saif
			'custom_theme_config', //by saif
			'button_condition', //by saif
			'custom_save', //by saif
			'detail_buttons_position', //by saif
			'relation_callback'); // by saif
    }
    ...
    protected function create_relation($name, $value = '', $tag = array(), $dependval = false)
    {
        ...
        if ($options)
        {
            foreach ($options as $opt)
            {
				...
				//by saif
				if(isset($this->relation_callback[$name])){
					$path = $this->check_file($this->relation_callback[$name]['path'],'relation_callback');
					include_once ($path);
					$output = call_user_func_array($this->relation_callback[$name]['callback'],array($opt['name']));
				}else{
					$output = $opt['name'];
				}
                $out .= $this->open_tag($opt_tag) . $this->html_safe($output) . $this->close_tag($opt_tag); // edit by saif
            }
        }
        $out .= $this->close_tag($tag);
        unset($options);
        ...
        $watermark_position = array(50, 50))
    {
        ...
        {
            $ratio = (($new_size_w ? $srcWidth : $srcHeight) / ($new_size_w ? (float)$new_size_w : (float)$new_size_h));
            $ratio = max($ratio, 1.0);
            $destWidth = ($srcWidth / $ratio);
            $destHeight = ($srcHeight / $ratio);
            if ($destHeight > $new_size_h)
            {
                $ratio = ($destHeight / ($new_size_h ? (float)$new_size_h : (float)$new_size_w));
                $ratio = max($ratio, 1.0);
        ...
        {
            $ratio = (($new_size_h ? $srcHeight : $srcWidth) / ($new_size_h ? (float)$new_size_h : (float)$new_size_w));
            $ratio = max($ratio, 1.0);
            $destWidth = ($srcWidth / $ratio);
            $destHeight = ($srcHeight / $ratio);
            if ($destWidth > $new_size_w)
            {
                $ratio = ($destWidth / ($new_size_w ? (float)$new_size_w : (float)$new_size_h));
                $ratio = max($ratio, 1.0);
                $destWidth = ($destWidth / $ratio);
                $destHeight = ($destHeight / $ratio);
            }
    ...
    protected function _render_list_buttons(&$row)
    {
        ...
        if ($this->buttons)
        {
            ...
            {
				if(isset($this->button_condition[$button['name']])){
					$path = $this->check_file($this->button_condition[$button['name']]['path'],'button_condition');
						include_once ($path);
						$condition = call_user_func_array($this->button_condition[$button['name']]['callback'],array($row));
				}else{
					$condition = true;
				}
                if ($this->is_button($button['name'], $row) && $condition)
                {
                    //$href = '';
                    /*if ($button['params'])
                    {
    ...
    protected function _check_unique_value()
    {
        ...
        $fdata = $this->_parse_field_names($unique, '_check_unique_value',false,false); // false, false added by saif
        $out = array();
        $table_join = $this->_build_table_join();
        if ($this->primary_val)
        {
    ...
    { // loads theme configuration from ini file
        if (is_file(XCRUD_PATH . '/' . Xcrud_config::$themes_path . '/' . $this->theme . '/xcrud.ini')){
			...
			//by saif
			foreach($this->custom_theme_config as $text=>$value){
				$this->theme_config[$text] = $value;
			}
			//end by saif
		}else
    ...
    public static function load_js()
    {
        ...
        $config = array(
            ...
            'lang' => self::$lang_arr,
			'showseconds' => Xcrud_config::$showseconds,
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        foreach ($this->fields_output as $key => $item)
        {
			...
			// by saif
			if (isset($this->row_classes[$item['name']]))
            {
                $row_class .= ' '.$this->row_classes[$item['name']];
            }
			// end by Saif
            ...
            '&uarr; ', 'desc' => '&darr; '))
    {
        ...
        {
            $out .= $this->open_tag($item,'xcrud-num') . '<input type="checkbox"  title="Select All" class="xcrud-select-all">' . $this->close_tag($item);
        }
		//end by Ahmed Saif
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
				...
				if ($this->is_selectable)
                {
                    $out .= $this->open_tag($item, 'xcrud-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) .
							'<input type="checkbox" title="select" class="xcrud-select" value="'.$row['primary_key'].'">' . $this->close_tag($item);
                }
				//end by saif
                ...
                foreach ($this->columns as $field => $fitem)
                {
                    ...
                    $out .= "<span class='responsive-title'>".$this->columns_names[$field].": </span>";  // added by saif
        ...
        if ($this->sum && $this->result_list)
        {
            ...
            foreach ($this->columns as $field => $fitem)
            {
                if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
                    'hidden'))
                    continue;
                $out .= $this->open_tag($item, isset($this->sum[$field]) ? $this->sum[$field]['class']." has-sum" : '', $this->_cell_attrib($field,
	...
	//by saif
	public function add_row_class($fields='',$class=''){
		$rows = $this->_parse_field_names($fields);
		foreach($rows as $row){
			$this->row_classes[$row['table'].'.'.$row['field']] = $class;
		}
	}
	
	public function custom_theme_config($text='',$value=''){
			$this->custom_theme_config[$text] = $value;
	}
	public function relation_format($fields='',$callback,$path='functions.php'){
		$rows = $this->_parse_field_names($fields);
		foreach($rows as $row){
			$this->relation_callback[$row['table'].'.'.$row['field']]['callback'] = $callback;
			$this->relation_callback[$row['table'].'.'.$row['field']]['path'] = $path;
		}
	}
	public function condition_button($button_name='',$callback,$path='functions.php'){
		$this->button_condition[$button_name]['callback']=$callback;
		$this->button_condition[$button_name]['path']=$path;
	}
	//end by saif
