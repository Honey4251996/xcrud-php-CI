...
/*else {
	...
	init_datepicker_range: function(type, container) {
		...
		var datepicker_config = {
			changeMonth: true,
			changeYear: true,
			showSecond: Xcrud.config('showseconds'),
	...
	init_texteditor: function(container) {
		...
		var elements_ar = jQuery(container).find(".xcrud-texteditor-ar:not(.editor-loaded)");
		if (jQuery(elements).size()) {
			if (Xcrud.config('editor_url') || Xcrud.config('force_editor')) {
				...
				//jQuery(elements_ar).addClass("editor-loaded").addClass("editor-instance-ar");
					...
					window.setTimeout(function() {
						jQuery.ajax({
							...
							success: function(js) {
								...
								//jQuery(elements_ar).removeClass("editor-instance-ar");
							},
							cache: true
						});
					}, 300);
			...
			if (Xcrud.config('editor_url') || Xcrud.config('force_editor')) {
				...
				if (Xcrud.config('editor_init_url')) {
					...
					if (typeof(tinyMCE) != 'undefined') {
						tinyMCE.init({
							mode: "textareas",
							editor_selector: "editor-instance-ar",
							content_style: "body {direction:rtl;}",
							menubar: false,
//							toolbar: "styleselect | bold | link | bullist | numlist | code",
//							plugins: [
//								 "advlist autolink link image lists charmap print preview hr anchor pagebreak spellchecker",
...
//							],
						...
						$('.editor-instance-ar').each(function(){
							var name = $(this).attr('name');
							CKEDITOR.replace(name,{
					...
					jQuery(elements).removeClass("editor-instance-ar");
				}
			}
		}
	},
	upload_file: function(element, data, container) {
		...
		jQuery(document).trigger("xcrudbeforeupload", [element,container, data]);
		//saif---------------------------- [element, is added by saif]
